...
// Simple helper to generate a random state (CSRF protection)
function generateState() {
  return crypto.randomUUID();
}

// Optional: basic redirect URI allowlist check
...
serve(async (req) => {
  ...
  try {
    ...
    if (!redirectUri || !isValidRedirectUri(redirectUri)) {
      return new Response(
        JSON.stringify({ error: "Invalid redirectUri" }),
    ...
    if (!clientId.includes(".apps.googleusercontent.com")) {
      ...
      return new Response(
        JSON.stringify({ error: "Invalid GOOGLE_CLIENT_ID format", details: hint }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
