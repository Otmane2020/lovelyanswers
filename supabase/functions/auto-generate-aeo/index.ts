...
serve(async (req) => {
  ...
  try {
    ...
    const opportunityQuestions = generateInitialQuestions(brandName, project.business_description || "", language);

    // Insert answers
    const answersToInsert = opportunityQuestions.slice(0, 5).map((q) => ({
      project_id: project.id,
      question: q.question,
      answer: generatePlaceholderAnswer(q.question, brandName, language),
