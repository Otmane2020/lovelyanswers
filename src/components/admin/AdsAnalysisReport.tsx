...
import { 
  ...
  ArrowRight, Lightbulb, Target, DollarSign, Loader2
} from "lucide-react";
...
function parseSeverity(title: string): "critical" | "important" | "improvement" | "info" {
  ...
  if (lower.includes("🔴") || lower.includes("critique") || lower.includes("critical")) return "critical";
  if (lower.includes("🟡") || lower.includes("important")) return "important";
  if (lower.includes("🟢") || lower.includes("amélioration") || lower.includes("improvement")) return "improvement";
  return "info";
}
...
function parseMarkdownSections(text: string): { summary: string; sections: ReportSection[] } {
  ...
  for (const line of lines) {
    // Detect H3 sections (### ) as recommendation blocks
    if (line.startsWith("### ")) {
      if (currentSection) {
      ...
      // H2 that's a recommendation section
      if (currentSection) {
        currentSection.content = contentBuffer.join("\n").trim();
        currentSection.actions = extractActions(currentSection.content);
        sections.push(currentSection);
...
function extractActions(content: string): string[] {
  ...
  for (const line of lines) {
    ...
    // Match action items: lines starting with *, -, or numbered, containing "Action" or imperative verbs
    if (
      (trimmed.startsWith("*   **Action") || trimmed.startsWith("- **Action") || trimmed.startsWith("*  **Action")) ||
      (trimmed.match(/^\d+\.\s+\*\*/) && (trimmed.includes("Action") || trimmed.includes("Mettre") || trimmed.includes("Ajouter") || trimmed.includes("Optimiser") || trimmed.includes("Vérifi") || trimmed.includes("Créer") || trimmed.includes("Supprim") || trimmed.includes("Paus")))
    ) {
      const cleaned = trimmed
        ...
        .replace(/^Action concrète\s*:\s*/i, "")
        .trim();
      if (cleaned.length > 10) actions.push(cleaned);
    }
  }
...
export function AdsAnalysisReport({ text, isStreaming }: AdsAnalysisReportProps) {
  if (!text && isStreaming) {
    return (
                      ...
                      if (trimmed.startsWith("*") || trimmed.startsWith("-")) {
                        ...
                        return (
                          <div key={i} className="flex items-start gap-2 pl-1">
                            ...
                            <span>{renderMarkdownLine(text)}</span>
                          </div>
                        );
                      }
                      if (trimmed.match(/^\d+\./)) {
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                          <Target className="h-3 w-3" /> Actions à mener
                        </p>
                        {section.actions.map((action, ai) => (
                          <div key={ai} className="flex items-start gap-2 bg-background/80 rounded-lg p-2.5 border border-border/50">
                            <CheckCircle2 className="h-4 w-4 mt-0.5 shrink-0 text-primary" />
                            <span className="text-xs leading-relaxed">{renderMarkdownLine(action)}</span>
                          </div>
                        ))}
                      </div>
                    </>
...
function extractKPIs(summary: string): { label: string; value: string; status?: "bad" | "ok" | "good" }[] {
  ...
  // Conversions
  const convMatch = summary.match(/(\d+)\s*conversion/i) || summary.match(/\*\*(\d+)\s*conversion/i);
  if (convMatch) {
