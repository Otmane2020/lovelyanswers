...
export const ExitIntentPopup = ({
  ...
  // Mobile: trigger on scroll-up after delay
  useEffect(() => {
    ...
    // Mobile: show after 30s of inactivity or fast scroll up
    let lastScrollY = window.scrollY;
    let mobileTimer: ReturnType<typeof setTimeout>;

    const handleScroll = () => {
      const currentY = window.scrollY;
      // Fast scroll up (intent to leave)
      if (lastScrollY - currentY > 200 && !sessionStorage.getItem("exit_intent_shown")) {
        setShow(true);
        sessionStorage.setItem("exit_intent_shown", "true");
        ...
        <>
          ...
          >
            <div className="relative w-full sm:max-w-md bg-background rounded-t-2xl sm:rounded-2xl border border-border shadow-2xl overflow-hidden">
              {/* Gradient header */}
              <div className="bg-gradient-to-r from-primary to-primary/80 px-4 py-6 sm:px-6 sm:py-8 text-center">
                <motion.div
                  animate={{ rotate: [0, -10, 10, -10, 0] }}
                  transition={{ duration: 0.5, delay: 0.3 }}
                >
                  <Gift className="h-10 w-10 sm:h-12 sm:w-12 text-primary-foreground mx-auto mb-2 sm:mb-3" />
                </motion.div>
              ...
              <div className="px-4 py-5 sm:px-6 sm:py-6 text-center">
                ...
                <Button
                  onClick={handleCta}
                  size="lg"
                  className="w-full gap-2 text-sm sm:text-base font-semibold"
                >
                  {ctaLabel}
                ...
                <button
                  onClick={() => setShow(false)}
                  className="mt-3 text-xs sm:text-sm text-muted-foreground hover:text-foreground transition-colors"
                >
                  No thanks, I'll pass
                </button>
              </div>
