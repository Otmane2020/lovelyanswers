...
const PLATFORM_GUIDES: Record<string, { title: string; steps: string[] }> = {
  ...
  wix: {
    title: "How to Connect Wix with OAuth (Headless)",
    steps: [
      "Go to your Wix Dashboard → Settings → Headless Settings",
      "Click 'Create OAuth App' or select an existing one",
      "Copy the 'Client ID' (format: 92ef2ed2-d432-...)",
      "In 'Authorized Redirect URIs', add your app's callback URL",
      "Ensure Blog permissions are enabled in the app settings",
      "The Site ID is visible in your Wix dashboard URL",
    ],
  },
  webflow: {
...
const CMS_CONFIG: Record<string, {
  ...
  wix: {
    ...
    description: "Publish blog posts to your Wix site using OAuth (Headless).",
    helpText: "Use Wix Headless with OAuth for secure connection.",
    tutorialUrl: "https://dev.wix.com/docs/sdk/backend/introduction",
    fields: [
