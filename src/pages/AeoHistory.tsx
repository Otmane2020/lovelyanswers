...
const platformLogos: Record<string, string> = { wordpress: wordpressLogo, shopify: shopifyLogo, wix: wixLogo };
                    ...
                    {articles.map((a) => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            {a.published_url && (<Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => window.open(a.published_url!, "_blank")} title="View on Site"><ExternalLink className="h-4 w-4 text-[hsl(222,47%,30%)]" /></Button>)}
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/articles/${a.id}`)} title="View Article"><Eye className="h-4 w-4" /></Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
